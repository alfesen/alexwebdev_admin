...
export class TechController {
  ...
  @UploadImage('icon')
  createTech(
    @Body() { heading, enText, plText, category }: CreateTechDto,
    @SharpImage(50, true) icon: string
  ) {
    return this.techService.createTech(heading, enText, plText, category, icon)
  }
  ...
  async updateTech(
    @Param('id') id: string,
    @Body() { heading, plText, enText, category }: CreateTechDto,
